...
const userDataRules = {
  login: {
    ...
    maxLength: helpers.withMessage(
      "Максимальная длинна 20 символа!",
      maxLength(20)
    ),
  },
  password: {
...
const login = async () => {
  ...
  loading.value = true;
  try {
    const res = await loginStore.login(userData.value);
    if (res) {
      useJwt.setAccessToken(res.access_token);
      useJwt.setRefreshToken(res.refresh_token);
      useJwt.setUserData(res.data);
...
onMounted(async () => {
  await branchesStore.fetchAllBranches();
});
</script>
        ...
        <div class="flex h-screen py-5 my-10 xl:h-auto xl:py-0 xl:my-0">
          ...
          >
            ...
            <div class="mt-2 text-center intro-x text-slate-400 xl:hidden">
              Пару кликов чтобы войти в свой аккаунт. Совершайте полеты в любую
              точку мира с нами.
            </div>
            <div class="mt-8 intro-x">
              <FormInput
                ...
                :class="{ 'border-red-400': $v.login.$errors.length }"
                placeholder="Логин"
